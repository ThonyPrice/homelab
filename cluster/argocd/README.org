...
2. [@2] Grab ArgoCD's initial password
#+BEGIN_SRC sh
nix-shell -p argocd --run "argocd admin initial-password -n argocd"
#+END_SRC

3. [@3] Port-forward to ArgoCD's API server, authenticate the CLI, and update password
#+BEGIN_SRC sh
...
4. [@4] Create the first ArgoCD Application resource ~argocd-apps~ that includes the definition of ArgoCD itself
#+BEGIN_SRC sh
